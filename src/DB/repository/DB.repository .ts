...
import { DeleteResult, GetLeanResultType, HydratedDocument, MongooseBaseQueryOptions, ProjectionType, QueryOptions, QueryWithHelpers, RootFilterQuery, Types, UpdateQuery, UpdateResult } from "mongoose";
...
export class dbRepository<TDocument>{
 ...
 async paginate({
      ...
      const docs = await this.model.find(filter, select || null, finalOptions).lean({ virtuals: true })as TDocument[];
